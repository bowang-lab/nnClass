...
class nnUNetCLSTrainer(nnUNetTrainer):
    def initialize(self):
        if not self.was_initialized:
            ...
            self._best_acc = None
            self.patient = self.current_patient = 50
    ...
    def on_epoch_end(self):
        ...
        if self._best_ema is None or self.logger.my_fantastic_logging['val_auc'][-1] > self._best_ema:
            ...
            improve = True
            self.current_patient = self.patient  # reset patient if we improved
        ...
        if not improve:
            ...
            self.print_to_log_file(f"Patient: {self.current_patient}/{self.patient}")
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            if self.current_patient == 0:
                self.print_to_log_file("Early stopping!")
                break
